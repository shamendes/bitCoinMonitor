CREATE VIEW VW_CONSULTAS_POR_HORA
AS
SELECT	C.*
FROM	(
		...
		FROM	(
				...
				FROM	TB_CONSULTA DAT INNER JOIN SIS_TB_HORAS HOR
							ON(1=1)
				)  DAT INNER JOIN TB_CONSULTA CON
					ON(	CON.dat_consulta <= DAT.DAT_BASE)
		GROUP BY DAT.DAT_BASE
		) D INNER JOIN TB_CONSULTA  C
			ON(	C.idt_consulta = D.idt_consulta)
UNION
SELECT	C.*
FROM	TB_CONSULTA C INNER JOIN ( SELECT MAX(IDT_CONSULTA) IDT_CONSULTA FROM TB_CONSULTA) MAX_CON
