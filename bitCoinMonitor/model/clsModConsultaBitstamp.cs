...
namespace bitCoinMonitor.model
{
    class clsModConsultaBitstamp
    {
        ...
        public clsModConsultaBitstamp(clsApiTickerBitstamp aObjTicker)
        {

            try
            {
                this.pCompra = aObjTicker.ticker.ask; //--invertido propositalmente
                ...
                this.pVenda = aObjTicker.ticker.bid; //--invertido propositalmente
                this.pVolume = aObjTicker.ticker.volume;
        ...
        public clsModConsultaBitstamp(long aIntID)
        {
            try
            {
                instanciarConexao();
        ...
        {
            const string cStrSQL = "SELECT MAX(idt_consulta) idt_consulta FROM TB_CONSULTA_BITSTAMP";
            long vLngRetorno = 0;
        ...
        {
            const string cStrSQL = "SELECT * FROM TB_CONSULTA_BITSTAMP WHERE IDT_CONSULTA = @ID";
        ...
        public int incluir()
        {
            const string cStrSqlInsert = "INSERT INTO TB_CONSULTA_BITSTAMP VALUES(@Data, @Maximo,@Minimo,@Ultimo,@Volume,@Compra,@Venda)";
        ...
        {
            const string cStrSQL = "SELECT * FROM VW_CONSULTAS_POR_HORA_BITSTAMP WHERE DAT_CONSULTA BETWEEN @DataInicio AND @DataFim ORDER BY IDT_CONSULTA DESC";
